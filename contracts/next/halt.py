...
def _pay(who, amount: int) -> None:
    """Send value to a plain address.

    In v0.2 this needed a declared EVM interface. A proxy does it directly now,
    and the warning that came with the old one still stands: this is for
    ordinary addresses. Pointing it at an Intelligent Contract ends the
    transaction with an error the surrounding try cannot catch, which cost a
    deployment to learn.
    """
    gl.contract.get_at(who).emit_transfer(amount)
...
def _balance_of(address: str) -> int:
    ...
    """
    try:
        return int(gl.contract.get_at(gl.Address(address)).balance)
    except Exception:
        return -1
...
def _witness_says(witness: str, target: str) -> str:
    ...
    **It is the accused's own witness and the round is told so.** That is not a
    ...
    """
    try:
        return _clip(str(gl.contract.get_at(gl.Address(witness)).view().report(target)),
                     MAX_WITNESS)
    except Exception:
        return ""
...
def _record_of(address: str) -> str:
    ...
    """
    try:
        answer = str(gl.contract.get_at(gl.Address(address)).view().entries(str(MOVEMENTS)))
    except Exception:
        return ""
    try:
...
class Halt(gl.contract.Contract):
    # One entry per protected contract, keyed by its address.
    guards: gl.storage.TreeMap[str, str]
    targets: gl.storage.DynArray[str]
    ...
    alarms: gl.storage.DynArray[str]
    ...
    def raise_alarm(self, target: str, evidence: str) -> str:
        ...
        whole = ""
        try:
            whole = str(gl.contract.get_at(gl.Address(address)).view().status())
        except Exception:
            whole = ""
        ...
        if not reading:
            ...
            if back > 0:
                _pay(gl.message.sender_address, back)
    ...
    def appeal(self, target: str, answer: str) -> str:
        ...
        if paid > 0:
            _pay(gl.message.sender_address, paid)
    ...
    def retire(self, target: str) -> str:
        ...
        self.guards[address] = json.dumps(record)
        if left > 0:
            _pay(gl.message.sender_address, left)
