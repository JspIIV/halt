...
class Prosecutor(gl.Contract):
    ...
    def write_one(self, guardian: str, target: str) -> str:
        ...
        line = ""
        try:
            said = json.loads(str(gl.get_contract_at(Address(keeper)).view().guard(address)))
            if said.get("ok"):
        ...
        facts = ""
        try:
            facts = str(gl.get_contract_at(Address(address)).view().status())
        except Exception:
            facts = ""
        if not facts:
