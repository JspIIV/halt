...
async function claim(vault, file, label, expect) {
  ...
  record({ label, kind: 'alarm', batch: 'balance_read_whole', at: new Date().toISOString(),
...
log('the protocol that misreports its own figures, three times');
for (let i = 1; i <= 3; i++) {
  await claim(await misreportingVault(), 'true_breach',
              `misreporting protocol, true claim ${i} of 3`, 'UPHELD');
}

log('\ntwo false claims against a protocol whose report has been discredited');
for (let i = 1; i <= 2; i++) {
  await claim(await misreportingVault(), 'u_exaggerated',
              `misreporting protocol, false claim ${i} of 2`, 'REFUSED');
}

log('\nfour false claims where the report is honest and must still protect');
for (let i = 1; i <= 4; i++) {
  await claim(await apartVault(), 'u_false_timing',
              `honest report, false timing claim ${i} of 4`, 'REFUSED');
}

log('\ntwo claims of a withdrawal that never happened');
for (let i = 1; i <= 2; i++) {
  await claim(await quietlyHonestVault(), 'c_contradicted',
              `honest report, invented withdrawal ${i} of 2`, 'REFUSED');
}

log('\nthree true claims on the actor line');
for (let i = 1; i <= 3; i++) {
  await claim(await lockstepVault(), 'k_plain',
              `honest report, true actor claim ${i} of 3`, 'UPHELD');
}

log('\ntwo true claims on the numeric line');
for (let i = 1; i <= 2; i++) {
  await claim(await breachedVault(), 'b_plain',
              `honest report, true numeric claim ${i} of 2`, 'UPHELD');
}

const all = JSON.parse(fs.readFileSync('results/trials.json', 'utf8'))
  .filter(t => t.batch === 'guardian_reads_the_balance');
