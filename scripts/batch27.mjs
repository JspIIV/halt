...
async function claim(vault, file, label, expect) {
  ...
  record({ label, kind: 'alarm', batch: 'chronological_ledger', at: new Date().toISOString(),
...
log('six false timing claims, against a ledger that reads in the order it happened');
for (let i = 1; i <= 6; i++) {
  await claim(await apartVault(), 'u_false_timing',
              `chronological ledger, false timing claim ${i} of 6`, 'REFUSED');
}

log('');
log('three true claims on the actor line');
for (let i = 1; i <= 3; i++) {
  await claim(await lockstepVault(), 'k_plain',
              `chronological ledger, true actor claim ${i} of 3`, 'UPHELD');
}

log('');
log('two on the protocol that misreports, which must stay caught');
for (let i = 1; i <= 2; i++) {
  await claim(await misreportingVault(), 'true_breach',
              `chronological ledger, misreporting protocol ${i} of 2`, 'UPHELD');
}

const all = JSON.parse(fs.readFileSync('results/trials.json', 'utf8'))
  .filter(t => t.batch === 'chronological_ledger');
