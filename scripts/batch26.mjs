...
async function claim(vault, file, label, expect) {
  ...
  record({ label, kind: 'alarm', batch: 'quieter_prompt', at: new Date().toISOString(),
...
log('six false timing claims, the case the last build broke');
for (let i = 1; i <= 6; i++) {
  await claim(await apartVault(), 'u_false_timing',
              `quieter prompt, false timing claim ${i} of 6`, 'REFUSED');
}

log('');
log('three true claims on the protocol that misreports, the hole that was closed');
for (let i = 1; i <= 3; i++) {
  await claim(await misreportingVault(), 'true_breach',
              `quieter prompt, misreporting protocol ${i} of 3`, 'UPHELD');
}

log('');
log('three true claims on the actor line, which must survive all of it');
for (let i = 1; i <= 3; i++) {
  await claim(await lockstepVault(), 'k_plain',
              `quieter prompt, true actor claim ${i} of 3`, 'UPHELD');
}

const all = JSON.parse(fs.readFileSync('results/trials.json', 'utf8'))
  .filter(t => t.batch === 'quieter_prompt');
